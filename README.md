...
* **Tyre Temperature System** - Real-time tire temperature simulation with 4 states (cold, suboptimal, optimal, overheated) affecting grip and wear
...
### Tyre Temperature System

Tires have realistic thermal physics with individual temperatures for each corner (Front Left, Front Right, Rear Left, Rear Right). For HUD clarity and general strategy, the **Live Stats panel and Race Engineer use the average temperature** of all four corners to determine the car's performance state. Temperature affects both **grip** and **wear rate**.

#### Temperature States

| State | Temperature | Engineer Message | Grip Effect |
|-------|-------------|------------------|-------------|
| **Cold** | <70°C | "Tires are cold... we need to warm them up" | -12% grip (0.88x) |
| **Suboptimal** | 70-85°C | "Tires are coming up to temperature..." | -4% grip (0.96x) |
| **Optimal** | 85-100°C | "Tires are now in the optimal window..." | Maximum grip (1.0x) |
| **Overheated** | >100°C | "Tires are overheating!" | -10% grip (0.90x) |

#### Temperature Wear Multipliers

The simulation uses a **U-shaped wear curve** - tires degrade faster when outside their thermal window:

| Temperature Range | Wear Factor | Physics Explanation |
|-------------------|-------------|---------------------|
| **<70°C (Cold)** | 1.4x | Compound too stiff, causes "graining" and tearing |
| **70-80°C (Suboptimal)** | 1.15x | Below optimal elasticity |
| **80-95°C (Sweet Spot)** | 1.0x | Optimal rubber flexibility and bite |
| **95-100°C (Hot)** | 1.25x | Approaching thermal degradation |
| **>100°C (Overheated)** | 1.6x | Compound breaks down, causes blistering |

#### Thermal Behavior

- **Warm-up Time**: Tires start at ambient (60°C) and typically take 1-2 laps to reach the sweet spot.
- **Heating Sources**: Heat is generated by speed (friction), heavy braking (front bias), acceleration (rear bias), and high cornering loads.
- **Cooling**: Tires cool naturally through airflow, increasing with speed.
- **Pit Stops**: Fresh tires are fitted at ambient temperature, requiring a new warm-up cycle.

#### How Setup Affects Thermal Behavior

Tire temperatures are a dynamic consequence of your vehicle configuration:

- **Braking Intensity**: Higher braking power increases heat generation in the front tires during deceleration.
- **Acceleration Load**: Aggressive engine acceleration ratings drive more heat into the rear tires.
- **Aerodynamics & Grip**: High downforce and cornering grip allow for higher speeds through turns, increasing lateral friction and overall thermal load.
- **Compound Characteristics**: Soft compounds reach their optimal window faster but are more sensitive to high-load heating. Hard compounds have higher thermal inertia and stability.

The Race Engineer announces temperature status changes to help you understand your tires' current performance state and how they are responding to the track conditions.
